...
{
#if defined(__linux__)
  return std::make_unique<PortHandlerLinux>(port_name);
#elif defined(__APPLE__)
  return std::make_unique<PortHandlerMac>(port_name));
#elif defined(_WIN32) || defined(_WIN64)
  return std::make_unique<PortHandlerWindows>(port_name));
#elif defined(ARDUINO) || defined(__OPENCR__) || defined(__OPENCM904__) || defined(ARDUINO_OpenRB)
  return std::make_unique<PortHandlerArduino>(port_name));
#endif
}
